...
local function JADLOG(text)
	...
	out("#### Dynamic Difficulty line 51: Jadlog_Filename is " .. tostring(Jadlog_Filename) .. " . popLog is " ..
		tostring(popLog) .. " ####")
...
local function get_empire_score_player()
	...
	elseif (subculture == "wh_main_sc_nor_norsca") then
		...
		for i = 0, owned_regions:num_items() - 1 do
			...
			if (region:province_name() == "wh3_main_combi_province_albion" or region:province_name() ==
				"wh3_main_combi_province_vanaheim_mountains" or region:province_name() ==
				"wh3_main_combi_province_helspire_mountains" or region:province_name() ==
				"wh3_main_combi_province_ice_tooth_mountains" or region:province_name() ==
				"wh3_main_combi_province_mountains_of_naglfari" or region:province_name() ==
				"wh3_main_combi_province_trollheim_mountains" or region:province_name() ==
				"wh3_main_combi_province_mountains_of_hel" or region:province_name() ==
				"wh3_main_combi_province_gianthome_mountains" or region:province_name() ==
	...
	elseif (subculture == "wh_dlc05_sc_wef_wood_elves") then
		...
		for i = 0, owned_regions:num_items() - 1 do
			local region = owned_regions:item_at(i)
			if (region:name() == "wh3_main_combi_region_kings_glade" or region:name() ==
				"wh3_main_combi_region_vauls_anvil_loren" or region:name() == "wh3_main_combi_region_waterfall_palace" or
				region:name() == "wh3_main_combi_region_crag_halls_of_findol" or region:name() ==
				"wh3_main_combi_region_the_witchwood" or region:name() == "wh3_main_combi_region_gaean_vale" or
				region:name() == "wh3_main_combi_region_laurelorn_forest" or region:name() ==
				"wh3_main_combi_region_gryphon_wood" or region:name() == "wh3_main_combi_region_forest_of_gloom" or
				region:name() == "wh3_main_combi_region_oreons_camp" or region:name() ==
				"wh3_main_combi_region_the_haunted_forest" or region:name() == "wh3_main_combi_region_jungles_of_chian" or
	...
	elseif (subculture == "wh_main_sc_chs_chaos") then
		...
		for i = 0, factions:num_items() - 1 do
			local current_faction = factions:item_at(i);
			if current_faction:is_vassal_of(human_faction) then
	...
	function(context)
		...
		if current_faction_name ~= "rebels" then
			cm:callback(function()
				...
				if (current_faction:is_vassal_of(human_faction) == true or current_faction:allied_with(human_faction) ==
					true) then
				...
				local effect_strength_construction = math.ceil(
					ai_buff_level * -3 * mod_construction * mod_extra_difficulty - 5)
				...
				effect_bundle:add_effect("wh_main_effect_economy_gdp_mod_all", "faction_to_region_own",
					effect_strength_tax_rate)
				effect_bundle:add_effect("wh_main_effect_building_construction_cost_mod", "faction_to_region_own",
					effect_strength_construction)
				effect_bundle:add_effect("wh_main_effect_province_growth_other", "faction_to_province_own",
					effect_strength_growth)
				effect_bundle:add_effect("wh_main_effect_agent_action_outcome_parent_army_xp_gain_factionwide",
					"faction_to_force_own", xp_gain_per_turn)
				effect_bundle:add_effect("wh_main_effect_force_all_campaign_experience_base_all", "faction_to_force_own",
					recruit_rank)
				effect_bundle:add_effect("wh_main_effect_force_all_campaign_post_battle_loot_mod",
					"faction_to_faction_own",
					effect_strength_battle_loot)
				if (cm:get_saved_value("jdyndif_enable_replenishment")) then
					effect_bundle:add_effect("wh2_main_effect_replenishment_characters", "faction_to_force_own",
						replenishment_bonus)
					effect_bundle:add_effect("wh_main_effect_force_all_campaign_replenishment_rate",
						"faction_to_force_own",
						replenishment_bonus)
				end
				if (cm:get_saved_value("jdyndif_enable_recruitment")) then
					effect_bundle:add_effect("wh_main_effect_unit_recruitment_points", "faction_to_province_own",
				...
				if (current_faction_name == "wh_main_emp_empire") or (current_faction_name == "wh_dlc07_vmp_von_carstein") then
					jlog("Turn: # " .. turn_number .. " |  Difficulty: " .. player_difficulty .. " | Player Score: " ..
						player_score .. "| AI Faction: " .. tostring(current_faction_name) .. " | AI Buff Level: " ..
						ai_buff_level .. " | Income: " .. "+" .. effect_strength_tax_rate .. "%" .. " | Growth: " ..
						"+" .. effect_strength_growth .. " | Construction Cost: " .. effect_strength_construction ..
						"%" .. " | Loot: " .. "+" .. effect_strength_battle_loot .. "%" .. " | Rank: " .. "+" ..
						recruit_rank .. " | Unit XP Per Turn: " .. "+" .. xp_gain_per_turn .. " | Replenishment: " ..
