...
function Scripted_HEF_Technology_Tree:start_technology_listeners()
	out(
		"#### HE_Tech_Expansion_JM: Scripted_HEF_Technology_Tree:start_technology_listeners Called. ####");
	...
	if cm:is_new_game() then
		for region_key, technologies in pairs(self.region_mapping) do
			...
			if region then
				out(
					"#### HE_Tech_Expansion_JM: New Game Detected. Checking Region: " ..
					tostring(region:name()) .. " Owned By Faction: " ..
					tostring(region:owning_faction():name()) ..
					" For Technology: " ..
					tostring(self.region_mapping[region:name()]) .. " ####")
	...
	core:add_listener("region_changed_technology_unlock",
		"RegionFactionChangeEvent", function(context)
			...
			if not region:is_abandoned() then
				...
				out(
					"#### HE_Tech_Expansion_JM: Region: " .. tostring(region:name()) ..
					" Is Abandoned Nobody Will Unlock Technologies From It.")
			end
			out("#### HE_Tech_Expansion_JM: Region: " .. tostring(region:name()) ..
				" Is Now Controlled By: " ..
				tostring(region:owning_faction():name()) ..
				" Enabling Technology: " ..
				tostring(self.region_mapping[region:name()]) ..
				" For Them. ####")
			self:toggle_technology(self.region_mapping[region:name()],
				owning_faction)
		end, true)

	core:add_listener("diplomatic_treaty_technology_unlock",
		"PositiveDiplomaticEvent", function(context)
			return
				context:is_military_alliance() or context:is_defensive_alliance() or
				context:is_vassalage()
		end, function(context)
			self:resolve_diplomatic_event(context:proposer(), context:recipient())
		end, true)

	core:add_listener("diplomatic_treaty_technology_unlock",
		"NegativeDiplomaticEvent", function(context)
			return context:was_military_alliance() or
				context:was_defensive_alliance() or context:was_vassalage()
		end, function(context)
			self:resolve_diplomatic_event(context:proposer(), context:recipient())
		end, true)
end

function Scripted_HEF_Technology_Tree:resolve_diplomatic_event(proposer,
															   recipient)
...
function Scripted_HEF_Technology_Tree:toggle_technology(technologies,
														region_owner)
	...
	if region_owner and not region_owner:is_rebel() then
		...
		local ally_list = {}
		for _, current_faction in model_pairs(
			region_owner:factions_allied_with()) do
		...
		for technology_key, details in pairs(technologies) do
			out(
				"#### HE_Tech_Expansion_JM:toggle_technology(): Checking To See If " ..
				tostring(region_owner:name()) .. " Is Culture " ..
				tostring(details.culture) .. " For Technology " ..
				tostring(technology_key) .. " ####")
			if faction_culture == details.culture then
				out(
					"#### HE_Tech_Expansion_JM:toggle_technology(): Unlocking Technology " ..
					tostring(technology_key) .. " For " ..
					tostring(region_owner:name()) .. " ####")
	...
	for _, current_faction in model_pairs(faction_list) do
		...
		for technology_key, details in pairs(technologies) do
			if current_faction_culture == details.culture and region_owner ~=
				current_faction and
				(not region_owner or (details.allow_allies and region_owner and
				not region_owner:is_ally_vassal_or_client_state_of(
					current_faction))) then
